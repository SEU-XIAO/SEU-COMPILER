...
static NFA buildNFAForRule(const std::string& suffix, int ruleId, int& stateCounter) {
    ...
    for (size_t i = 0; i < suffix.length(); i++) {
        ...
        if (c == '\\' && i + 1 < suffix.length()) {
            ...
            nfa.states[start].edges.insert(std::make_pair(actualChar, end));
        ...
        if (c == '@') {
            ...
            for (int endState : frag1.endStates) {
                nfa.states[endState].edges.insert(std::make_pair('\0', frag2.startState));
            }
            ...
            nfa.states[newStart].edges.insert(std::make_pair('\0', frag1.startState));
            nfa.states[newStart].edges.insert(std::make_pair('\0', frag2.startState));

            for (int endState : frag1.endStates) {
                nfa.states[endState].edges.insert(std::make_pair('\0', newEnd));
            }
            ...
            nfa.states[newStart].edges.insert(std::make_pair('\0', frag.startState));
            nfa.states[newStart].edges.insert(std::make_pair('\0', newEnd));

            for (int endState : frag.endStates) {
                nfa.states[endState].edges.insert(std::make_pair('\0', frag.startState));
                nfa.states[endState].edges.insert(std::make_pair('\0', newEnd));
            }
            ...
            for (int ch = 1; ch < 128; ch++) {
                nfa.states[start].edges.insert(std::make_pair((char)ch, end));
            }
            ...
            nfa.states[start].edges.insert(std::make_pair(c, end));
...
NFA convertToNFA() {
    ...
    for (const auto& nfa : nfas) {
        finalNFA.states[newStart].edges.insert(std::make_pair('\0', nfa.startState));
    }
